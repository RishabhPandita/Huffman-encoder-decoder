...
public class decoder {
	...
	public void createTree(HuffmanTreeStructure r, String x, String val) {
		int len = x.length();
		HuffmanTreeStructure t = r;
		int i = 0;
		while (i < len - 1) {
			char temp = x.charAt(i++);
			if (temp == '0') {
				if (t.left == null) {
					HuffmanTreeStructure tree = new HuffmanTreeStructure(Integer.MIN_VALUE, 0, null, null);
					t.left = tree;
				}
				t = t.left;
		...
		HuffmanTreeStructure tree = new HuffmanTreeStructure(Integer.parseInt(val), 0, null, null);
		if (x.charAt(i) == '1') {
			t.right = tree;
		} else {
			t.left = tree;
	...
	public void createDecodedText(HuffmanTreeStructure root, String encodedString, HashMap<String, String> map) {
		try {
			HuffmanTreeStructure t = root;
			...
			while (i < encodedString.length()) {
				if (t.data != Integer.MIN_VALUE) {
					...
					str.setLength(0);
					continue;
				} else if (encodedString.charAt(i) == '0') {
					str.append("0");
					t = t.left;
	...
	public static void main(String[] args) {
		...
		HuffmanTreeStructure root = new HuffmanTreeStructure(Integer.MIN_VALUE, 0, null, null);
		HashMap<String, String> map = new HashMap<String, String>();
		String encodedString = "";
		try {
			encodedString = decoder.getEncodedString(args[0]);
		} catch (Exception e) {
			System.out.println("Something went wrong!");
			return;
		}
